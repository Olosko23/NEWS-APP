import React from 'react'
 ...
 function Menu({active, setActive, setCategory}) {
  ...
  return (
      <nav className="w-full h-11 bg-slate-400">
        <ul className="flex gap-4">
          {links.map (link => (
            <li key={link.id} className={active === link.id ? "hover:scale-105 cursor-pointer active:font-semibold" : "font-normal cursor-pointer"} 
